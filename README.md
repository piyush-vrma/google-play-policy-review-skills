...
```
skills/google-play-policy-review/
...
└── evals/                       # Automated benchmark test suite & assertions
    ├── eval_metadata.json       # Benchmark configuration & rubrics
    ├── evals.json               # 4 comprehensive test scenarios
...
Contributions are warmly welcome! Check out [CONTRIBUTING.md](CONTRIBUTING.md) for full guidelines.
...
3. Run the evaluation suite with:
   ```bash
